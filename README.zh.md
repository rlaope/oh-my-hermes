...
`ultrabrain`、`deep`、`architect`、`unspecified-high`、`unspecified-low`、`quick`、`writing`、`visual-engineering`、`artistry`：每一个都是可编辑的模型 + effort 链，与下方“推荐模型”里列出的九个一致，在一个文件里即可查看和覆盖。提供方拒绝某个模型时链会前进；一次会继承无法提供该模型的提供方的派发会被拒绝，而不是悄悄降级。setup 会询问你的提供方，并按当前这台机器重排链。
