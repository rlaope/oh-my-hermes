...
class RouterContentTests(unittest.TestCase):
    ...
    def test_first_release_trust_surfaces_are_present(self) -> None:
        ...
        self.assertIn("Hermes-native path also", readme)
        ...
        self.assertIn("assets/omh-profile-interaction-map.svg", readme)
        ...
        self.assertIn("# check omh status\nomh doctor", quick_start)
        self.assertLess(quick_start.index("omh setup"), quick_start.index("# check omh status"))
        self.assertLess(quick_start.index("# check omh status"), quick_start.index("omh doctor"))
        ...
        self.assertIn("installation guide", quick_start)
        ...
        self.assertIn("oh_my_hermes-1.0.1-py3-none-any.whl", release)
        ...
        self.assertIn("The prompt builder is where Hermes becomes more than a chat window.", site_architecture_post)
        self.assertIn("Memory is not a drawer of notes; it is retrieval discipline.", site_architecture_post)
        self.assertIn("Skills are procedural memory.", site_architecture_post)
        self.assertIn("Cron and gateway support turn Hermes into an operator, not just an answer box.", site_architecture_post)
        self.assertIn("OMH belongs above this as a workflow and evidence lens", site_architecture_post)
        self.assertIn("it as a hidden executor.", site_architecture_post)
        self.assertIn("프롬프트 빌더에서 Hermes는 단순 채팅창을 넘어섭니다.", site_architecture_post)
        self.assertIn("메모리는 메모장 서랍이 아니라, 무엇을 꺼낼지 정하는 규칙입니다.", site_architecture_post)
        self.assertIn("스킬은 Hermes가 몸에 익히는 절차적 기억입니다.", site_architecture_post)
        self.assertIn("OMH는 숨은 실행기가 아니라 workflow와 evidence를 정리하는", site_architecture_post)
        ...
        self.assertIn("operating layer that makes Hermes work", site_architecture_post)
        self.assertIn("OMH is the workflow lens", site_architecture_post)
        ...
        self.assertIn("Myth: more commands equals", site_architecture_post)
        self.assertIn("a stronger product.", site_architecture_post)
        self.assertIn("오해: 명령어가 많을수록", site_architecture_post)
        self.assertIn("제품이 강하다.", site_architecture_post)
        self.assertIn("Use OMH web-research for: compare these three competitors and brief me.", site_architecture_post)
        self.assertIn("Use OMH loop for: improve this repo's first-run experience", site_architecture_post)
        self.assertIn("Use OMH img-summary for: make a PR summary card reviewers can read quickly.", site_architecture_post)
        self.assertIn("Use OMH request-to-handoff for: turn this issue into a PR-ready implementation path.", site_architecture_post)
