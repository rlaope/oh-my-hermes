...
class CliTests(unittest.TestCase):
    ...
    def test_release_product_readiness_cli_summarizes_product_story(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("96/96 route hints aligned", gates["route_hint_alignment"]["summary"])
            ...
            self.assertIn("21/21 interventions", gates["routing_precision"]["summary"])
    ...
    def test_release_evidence_bundle_cli_packages_optional_write_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "Routing precision: 39/39 negative controls, 21/21 interventions "
            ...
            self.assertEqual(payload["summary"]["route_hint_alignment_aligned"], 96)
            ...
            self.assertEqual(payload["summary"]["routing_precision_intervention_passing"], 21)
            self.assertEqual(payload["summary"]["routing_precision_intervention_total"], 21)
    ...
    def test_demo_route_hint_alignment_keeps_router_and_awareness_hints_in_sync(self) -> None:
        ...
        self.assertEqual(payload["summary"]["case_count"], 96)
        self.assertEqual(payload["summary"]["hinted_count"], 96)
        self.assertEqual(payload["summary"]["aligned_count"], 96)
        ...
        self.assertEqual(explicit_payload["summary"]["case_count"], 96)
    ...
    def test_demo_route_hint_alignment_summary_is_human_readable(self) -> None:
        ...
        self.assertIn("Result: 96/96 route hints aligned (all passing)", stdout)
        self.assertIn("Hints present: 96/96; missing hints: 0; mismatches: 0", stdout)
        self.assertIn(
