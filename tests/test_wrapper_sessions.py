...
class WrapperSessionTests(unittest.TestCase):
    ...
    def test_codex_wrapper_session_exposes_open_attach_record_actions(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(codex_launch["launch_owner"], "hermes_or_wrapper")
            self.assertEqual(codex_launch["configured_executor_profile"], "codex")
            self.assertFalse(codex_launch["ui_only"])
            self.assertEqual(codex_launch["execution_policy"], "hermes_or_wrapper_may_start_terminal_session")
    ...
    def test_prompt_only_executor_session_tracks_attached_result_without_runtime_run(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(claude_launch["launch_owner"], "hermes_or_wrapper")
            self.assertEqual(claude_launch["configured_executor_profile"], "claude-code")
            self.assertFalse(claude_launch["ui_only"])
            self.assertEqual(claude_launch["execution_policy"], "hermes_or_wrapper_may_start_terminal_session")
    ...
    def test_non_terminal_executor_launches_are_prompt_only(self) -> None:
        for executor in ("generic", "hermes", "omx-runtime", "omo-runtime", "omc-runtime"):
            with self.subTest(executor=executor):
                ...
                self.assertEqual(launch["execution_policy"], "prompt_or_runtime_handoff_only")
                self.assertEqual(launch["launch_owner"], "hermes_or_wrapper")
