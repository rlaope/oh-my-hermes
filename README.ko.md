...
OMH는 Hermes Agent를 위한 세 가지를 플러그인 하나로 전달한다. 코딩 인텔리전스(01–04, 07), 장기 기억 시스템(08), 최적화된 워크플로 패키지(05–06). 실제 화면을 기준으로 장면 하나씩.

### 01 · 모델별 최적화 튜닝, 작업 분할, 코딩 스킬 향상

OMH의 코딩 쪽은 세 가지다. 모델별로 프롬프트를 튜닝하고(03), 일을 병렬 lane으로 쪼개고(04), 요청이 부르는 전문가 스킬을 싣는다(06). 시작은 라우팅이다. 모든 요청은 디스패치 전에 점수가 매겨지고, 점수를 움직인 신호는 전부 이름이 붙는다. 이름 바꾸기는 light로 판정돼 quick 레인으로 가고, "X를 참조하는 곳을 전부 찾아라"는 exhaustive-search 신호에 걸려 하나도 빠뜨리지 않을 모델로 간다. 같은 코딩 문제를 같은 GPT-6 Astra로 잰 결과: 같은 답을 $4.29 대신 $0.66에, 23분 대신 5분에 얻었다.
