...
| `img-summary` | Prepare provider-neutral visual prompt cards for meeting notes, PR summaries, issue triage, research briefings, and release announcements, with source-specific formats, premium domain-aware background plate/texture/camera direction, image-tool setup fallback, and observed-only generation, QA, and delivery. | "Make a PR summary card for reviewers." |
