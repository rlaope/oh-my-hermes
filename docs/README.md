...
| `research-department` / `web-research` / `research-brief` / `report-package` | Prepare Scout -> Analyst -> Briefer research operations with source inbox, briefing status, and optional NotebookLM/Obsidian readiness. | "Every morning, watch competitor news and brief me if something changed." |
