...
| HUD, status, and session observability | Available | `omh hud`, plugin `omh_recommend`/`omh_probe`/`omh_hud`/`omh_status`, wrapper sessions, runtime runs, roadmap cards, and status cards. | Live host HUD rendering depends on Hermes/plugin support; plugin probe output is metadata/status, not execution evidence. |
